document.addEventListener('DOMContentLoaded', async () => {
    ...
    // ... (funções showCustomConfirm e initializePage sem alteração) ...
    ...
    function displayAdventureDetails(data) {
        ...
        // MUDANÇA: Usa a biblioteca marked para renderizar o Markdown em HTML
        const descriptionElement = document.getElementById('adventure-description');
        if (data.descricao) {
            // Usa .innerHTML para que o HTML gerado seja interpretado pelo navegador
            descriptionElement.innerHTML = marked.parse(data.descricao);
        } else {
            descriptionElement.textContent = 'Nenhuma descrição fornecida.';
        }
    }

    // ... (resto do arquivo sem alteração) ...

    // Para garantir, aqui está o código completo do aventura.js
    ...
    async function renderComments() {
        ...
        const { data: comments, error: commentsError } = await supabaseClient.from('comentarios').select('*, profile:profiles!left(username)').eq('aventura_id', adventureId).order('created_at', { ascending: true });
        if(commentsError) { console.error("Erro ao buscar comentários:", commentsError); return; }
